...
void AnalyseImage(KNearest knearest)
{
 ...
 vector < vector<Point> > contours;
 //image = imread("../../imagecapture/output.jpg", 1);
 image = imread("../images/output.jpg",1); 
 ...
 //  cout << " i = " << i << endl;
   ...
   if (rec.height > 28)
   {
    ...
    rectangle(image, Point(rec.x, rec.y),
      ...
      Scalar(0, 0, 255), 2);
    imshow("all", image);
    cout << "Read Node Number : "<< result << "\n\n";
    cout << "End of recogniion !"<<endl;
    waitKey(0);
   }
